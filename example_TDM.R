...
tdm.df <- as.data.frame(inspect(tdm), stringsAsFactors = FALSE)
write.csv(tdm.df, "mydata0.998sparse.csv")
tdm.df2 <- read.csv("mydata0.998sparse.csv", stringsAsFactors = FALSE)

# word counts (in )decreasing order):
word.freq = sort(rowSums(td.mat), decreasing = TRUE)

wf.dm = data.frame(word = names(word_freqs), freq = word.freq)
write.table(wf.dm, "wf_dm.txt", row.names = FALSE)
